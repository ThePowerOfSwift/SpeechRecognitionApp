    ...
    public static class FuzzyStringMatcher
    {
        public static bool IsSuitableString(string command, string recognized)
        {
            ...
            if (recognized.Contains(" "))
            {
                ...
                var resultWeak =
                    command.ApproximatelyEquals(recognized, FuzzyStringComparisonTolerance.Weak, options.ToArray());
                var resultNormal = command.ApproximatelyEquals(recognized, FuzzyStringComparisonTolerance.Normal,
                    options.ToArray());
            ...
            else
            {
                ...
                var result1 =
                    command.ApproximatelyEquals(recognized, FuzzyStringComparisonTolerance.Weak, options.ToArray());
                var result2 = command.ApproximatelyEquals(recognized, FuzzyStringComparisonTolerance.Normal,
                    options.ToArray());
